...
The repository owner and rightsholder supplied and authorized all six high-resolution PNG files in `assets/`, and their generated derivatives, for distribution under this repository's MIT License. The build preserves those source files and generates the runtime derivatives reproducibly. Placement icons, score artwork, plugin artwork, and store media are generated by `scripts/assets.mjs`. No third-party provenance is claimed for the supplied artwork.
